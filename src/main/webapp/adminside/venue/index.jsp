...
<head>
    <meta charset="UTF-8">
    <title>Manage Venues | Planzone Admin</title>
...
<main class="flex-1 p-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-orange-600">
            <i class="fa-solid fa-building mr-2"></i> Manage Venues
        </h1>
        <a href="${pageContext.request.contextPath}/admin/venue?action=create"
           class="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-600 transition">
            <i class="fa-solid fa-plus mr-1"></i> Create Venue
        </a>
    </div>

    <div class="mb-4">
        <input type="text" id="search" placeholder="Search venue..."
        ...
        <table class="w-full text-sm">
            ...
            <tbody id="venueTable">
                <c:forEach var="venue" items="${venues}">
                    <tr class="border-t">
                        <td class="py-3 px-4">${venue.venueId}</td>
                        <td class="py-3 px-4">${venue.name}</td>
                        <td class="py-3 px-4">${venue.address}</td>
                        <td class="py-3 px-4">${venue.capacity}</td>
                        <td class="py-3 px-4">
                            <a href="${pageContext.request.contextPath}/admin/venue?action=edit&id=${venue.venueId}"
                               class="text-blue-500 hover:text-blue-700 mr-3"><i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="javascript:void(0);" onclick="confirmDelete('${pageContext.request.contextPath}/admin/venue?action=delete&id=${venue.venueId}')"
